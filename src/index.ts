...
const DEFAULT_ABANDONED_THIRD_TEMPLATE = "ig_abandoned_checkout_r3_10off";
...
async function handleWhatsAppOrderFlow(
  ...
  if (!context) {
    ...
    const variants = allOrderVariants(product);
    ...
    if (!firstVariant) {
      await sendAllVariantOptions(env, phone, language, product, meaningfulVariants);
  ...
  if (context.step === "variant") {
    const variants = allOrderVariants(context.selected_product!).filter(
      (variant) => normalize(variant.title) !== "default title",
    );
    const selected = selectVariantFromReply(variants, text);
    if (!selected) {
      await sendAllVariantOptions(env, phone, language, context.selected_product!, variants);
      return true;
    }
  ...
  if (context.step === "customer_name") {
    ...
    context.step = "mobile";
    await saveOrderFlowContext(env, context);
    await replyAndLog(env, phone, mobileQuestionMessage(language));
    return true;
  }

  if (context.step === "mobile") {
  ...
  if (context.step === "confirm") {
    ...
    const checkoutUrl = directCheckout
      ? buildShopifyCheckoutUrl(env, context, phone)
      : buildManualOrderUrl(env, context);
    await saveWhatsAppOrderDraft(
      env,
      context,
      checkoutUrl,
      phone,
    ...
    if (!directCheckout) await createHumanHandoff(env, phone, "variant_manual_order");
    await replyAndLog(env, phone, orderReceivedMessage(language, context, checkoutUrl, directCheckout));
    await clearOrderFlowContext(env, phone);
    return true;
  }
...
export function allOrderVariants(product: ProductSuggestion): ProductVariantInfo[] {
  const variants = (product.variants ?? []).filter(
    (variant) => Boolean(String(variant.id ?? "").trim()),
  );
  if (variants.length > 0) return variants;
  const price = Number(product.price ?? product.price_min);
  return [{
    id: "",
...
function selectVariantFromReply(
  ...
  const normalizedValue = normalize(value);
  const number = /^(?:option\s*)?([1-9]\d{0,3})$/.exec(normalizedValue);
...
export function parseIndianMobile(value: string): string | null {
...
async function sendAllVariantOptions(
  env: Bindings,
  phone: string,
  language: Language,
  product: ProductSuggestion,
  ...
  const header = language === "en"
    ...
    : `*${product.title}* ke sabhi variants:\n`;
  const footer = language === "en"
    ? "\n✅ Available: direct payment link\n📝 Manual order: team verification\n\nReply with the option number."
    : "\n✅ Available variant par direct payment link milega.\n📝 Manual order variant bhi select kar sakte hain.\n\nSirf option number bhejein.";
  const chunks: string[] = [];
  let chunk = header;
  for (const line of lines) {
    if (chunk.length + line.length + footer.length + 2 > 3900 && chunk !== header) {
      chunks.push(chunk.trimEnd());
      chunk = `*${product.title}* variants continued:\n`;
    }
    chunk += `${line}\n`;
  }
  chunks.push(`${chunk.trimEnd()}${footer}`);
  for (const message of chunks) await replyAndLog(env, phone, message);
}
...
function canCreateDirectCheckout(variant: ProductVariantInfo | null): boolean {
  return Boolean(variant?.available && /^\d+$/.test(String(variant.id)));
}

function buildManualOrderUrl(env: Bindings, context: OrderFlowContext): string {
  const url = new URL(
    context.selected_product
      ? absoluteUrl(shopDomain(env), context.selected_product.url)
      : shopDomain(env),
  );
  const variantId = String(context.selected_variant?.id ?? "").replace(/\D/g, "");
  if (variantId) url.searchParams.set("variant", variantId);
  url.searchParams.set("utm_source", "whatsapp");
  url.searchParams.set("utm_medium", "manual_order");
  return url.toString();
}

function orderReceivedMessage(
  _language: Language,
  context: OrderFlowContext,
  checkoutUrl: string,
  directCheckout: boolean,
): string {
  const option = context.selected_variant?.title && normalize(context.selected_variant.title) !== "default title"
    ? `\nOption: ${context.selected_variant.title}`
    : "";
  const customization = context.customization_text ? `\nCustomization: ${context.customization_text}` : "";
  const heading = directCheckout ? "🔐 *Secure Payment Link:*" : "🔐 *Order & Payment Page:*";
  const followUp = directCheckout
    ? "Payment complete hone ke baad Shopify order number milega."
    : "IG Store team selected option verify karke final secure payment link confirm karegi.";
  return `✅ *Order Details Received*\n\nProduct: ${context.selected_product?.title || "Product"}${option}${customization}\nQuantity: ${context.quantity}\nCustomer: ${context.customer_name}\nMobile: +91 ${context.mobile_phone}\nAddress: ${context.full_address}\nPIN: ${context.pincode}\n\n${heading}\n${checkoutUrl}\n\n${followUp} OTP, UPI PIN, CVV ya card PIN WhatsApp par share na karein.`;
}
...
async function getOrderFlowContext(
  ...
  const row = await env.DB.prepare(`
    SELECT phone, step, selected_product_json, selected_variant_json,
           customization_text, quantity, customer_name, mobile_phone, full_address, pincode
  ...
  try {
    return {
      phone: row.phone,
      step: canonicalOrderStep(String(row.step || "")),
...
async function saveOrderFlowContext(env: Bindings, context: OrderFlowContext): Promise<void> {
  await env.DB.prepare(`
    INSERT INTO order_flow_context (
      ...
      customization_text, quantity, customer_name, mobile_phone, full_address, pincode, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
    ON CONFLICT(phone) DO UPDATE SET
      step = excluded.step,
...
async function saveWhatsAppOrderDraft(
  ...
  await env.DB.prepare(`
    INSERT INTO whatsapp_order_drafts (
      phone, mobile_phone, customer_name, product_title, product_url, variant_id, variant_title,
...
export function canonicalOrderStep(step: string): string {
  const aliases: Record<string, string> = {
    wa_variant: "variant", wa_customization: "customization", wa_quantity: "quantity",
    wa_name: "customer_name", wa_mobile: "mobile", wa_address: "address",
    wa_pincode: "pincode", wa_confirm: "confirm",
    vx_variant: "variant", vx_customization: "customization", vx_quantity: "quantity",
    vx_name: "customer_name", vx_mobile: "mobile", vx_address: "address",
    vx_pincode: "pincode", vx_confirm: "confirm",
  };
  return aliases[step] ?? step;
}

function mobileQuestionMessage(language: Language): string {
  if (language === "en") {
    return "Send a 10-digit Indian mobile number for delivery and order confirmation (starting with 6, 7, 8 or 9).";
  }
  return "Delivery aur order confirmation ke liye 10-digit mobile number bhejein. Example: 9587666693";
}
